...
const headerInputStyle =
  "bg-[rgba(37,99,235,0.15)] border border-(--accent-blue)rounded text-(--text-primary) px-[0.4rem] py-[0.2rem] outline-none font-inherit focus:bg-[rgba(37,99,235,0.2)] ";
...
const ReviewPage = ({ onNext }: ReviewPageProps) => {
  ...
  const [canonical, setCanonical] = useState<Record<string, unknown> | null>(
    null,
  );
        ...
        while (retries < maxRetries && isMountedRef.current) {
          try {
            ...
            if (data && typeof data === "object") {
              ...
              const property = dataObj.property as
                | Record<string, unknown>
                | undefined;
              const location = dataObj.location as
                | Record<string, unknown>
                | undefined;
              const listingMeta = dataObj.listing_meta as
                | Record<string, unknown>
                | undefined;
            ...
            console.warn(
              `Failed to load canonical (attempt ${retries + 1}/${maxRetries}):`,
              errorMessage,
            );
            ...
            addToast({
              message:
                error instanceof Error
                  ? error.message
                  : "Failed to load listing data. Please check if the backend is running.",
              type: "error",
            }),
          );
        }
  ...
  const handleFieldChange = async (
    ...
    if (
      ...
      field.includes("foundation") ||
      field.includes("roof") ||
      field.includes("fireplaces") ||
      field.includes("appliances") ||
      field.includes("water") ||
      field.includes("sewer") ||
      field.includes("heating") ||
      field.includes("cooling") ||
      field.includes("flooring") ||
    ...
    // Handle number fields
      ...
      field.includes("spaces") ||
      field.includes("parking") ||
    ...
    // Handle boolean fields
      ...
      parsedValue =
        newValue.toLowerCase() === "yes" || newValue.toLowerCase() === "true";
    }
    // Handle date fields
    ...
    if (newEditMode) {
      setTimeout(() => {
        ...
        const firstInput = document.querySelector(
          ".scrollContainer input, .scrollContainer textarea",
      ...
      >
        <div
          className="
            ...
            gap-4"
        >
          <p className="text-(--text-primary)">Loading listing data...</p>
      ...
      >
        ...
        <div
          className="
            ...
            gap-4"
        >
          <p className="text-(--text-primary)">No data available</p>
          <p className="text-(--text-secondary) text-[0.85rem]">
  ...
  const listingMeta = canonical.listing_meta as
    | Record<string, unknown>
    | undefined;
  ...
  const expirationDate = formatDate(
    listingMeta?.expiration_date as string | null,
  );
  ...
  const sqft = property?.living_area_sqft
    ? String(property.living_area_sqft)
    : "-";
  ...
  const renderStatField = (
    ...
    if (isEditMode) {
      ...
      return (
        <div className="flex flex-col items-end gap-[2px] justify-center">
          <span className="text-[0.6rem] text-(--text-secondary) uppercase mt-[0.1rem]">
            {label}
          </span>
          <input
            className="
    ...
    return (
      <div className="flex flex-col items-end justify-center">
        <span className="text-[1rem] font-semibold text-(--text-primary) leading-[1.2]">
          {value}
        </span>
        <span className="text-[0.6rem] text-(--text-secondary) uppercase mt-[0.1rem]">
          {label}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col flex-1">
          {isEditMode ? (
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    if (total !== null && total > 0) {
                      setCanonicalValue(
                        updated,
                        ["property", "main_level_bedrooms"],
                        total,
                      );
                      setCanonicalValue(
                        updated,
                        ["property", "other_level_bedrooms"],
                        0,
                      );
                    } else {
                      setCanonicalValue(
            ...
            <>
              <h1 className="text-[1rem] font-semibold text-(--text-primary) mb-[0.1rem]">
                {streetAddress}
              </h1>
              <span className="text-[0.6rem] text-(--text-secondary) tracking-[0.05em] font-medium">
                {county.toUpperCase()}, {city.toUpperCase()},{" "}
                {state.toUpperCase()}
              </span>
              <span className="text-[0.55rem] text-(--text-secondary) opacity-70 mt-[0.1rem]">
        ...
        <div className="flex items-center gap-[1.2rem]">
          <div className="flex gap-[1.2rem] mr-[1.2rem] min-h-[2.2rem] items-center">
            {renderStatField("LIST PRICE", listPrice, "listPrice", [
              "listing_meta",
              "list_price",
            ])}
            {renderStatField("AGREEMENT TYPE", agreementType, "agreementType", [
              "listing_meta",
              "listing_agreement",
            ])}
            {renderStatField(
              ...
              expirationDate,
              "expirationDate",
              ["listing_meta", "expiration_date"],
            )}
            {renderStatField("YEAR BUILT", yearBuilt, "yearBuilt", [
              "property",
              "year_built",
            ])}
            {renderStatField("OWNERSHIP", ownership, "ownership", [
              "property",
              "ownership_type",
            ])}
          </div>
          ...
          <div className="flex gap-[0.6rem]">
            <button
              className={`
                        ...
                        cursor-pointer
                        hover:bg-white/10
                        ${isEditMode ? "bg-(--accent-blue) border-(--accent-blue) text-white" : ""}
                    `}
      ...
      <div
        className="
                ...
                "
      >
        <div className="grid grid-cols-[2.5fr_1fr] min-h-full">
          {/* Left Column: Topics 1-5 */}
          <div className="p-4 border-r border-(--card-border) bg-white/[0.015]">
            <TopicSection
              title="1. Agreement & Listing Services"
              defaultOpen={true}
            >
              <EditableField
              ...
              <EditableField
                ...
                value={
                  getFieldValue(["listing_meta", "special_conditions"]) ||
                  "None"
                }
                forceEdit={isEditMode}
                multiline={true}
                onSave={(v) =>
                  handleFieldChange(
                    "specialConditions",
                    ["listing_meta", "special_conditions"],
                    v,
                  )
                }
              />
            ...
            <TopicSection
              title="2. Property Identity & Location"
              defaultOpen={true}
            >
              <EditableField
                label="Zip Code"
                ...
                forceEdit={isEditMode}
                onSave={(v) =>
                  handleFieldChange("zipCode", ["location", "zip_code"], v)
                }
              />
              <EditableField
                label="Parcel Number"
              ...
              <EditableField
                ...
                onSave={(v) =>
                  handleFieldChange(
                    "legalDescription",
                    ["location", "tax_legal_description"],
                    v,
                  )
                }
              />
            ...
            <TopicSection
              title="3. Physical & Architectural Specs"
              defaultOpen={true}
            >
              <EditableField
              ...
              <EditableField
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("roofType", ["property", "roof"], v)
                }
              />
              <EditableField
            ...
            <TopicSection title="4. Interior & Rooms" defaultOpen={true}>
              ...
              <EditableField
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("fireplaces", ["features", "fireplaces"], v)
                }
              />
              <EditableField
                label="Appliances"
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("appliances", ["features", "appliances"], v)
                }
              />
              <EditableField
                label="Water Source"
              ...
              <EditableField
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("sewer", ["utilities", "sewer"], v)
                }
              />
              <EditableField
                label="Heating"
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("heating", ["utilities", "heating"], v)
                }
              />
              <EditableField
                label="Cooling"
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("cooling", ["utilities", "cooling"], v)
                }
              />
              <EditableField
                label="Flooring"
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("flooring", ["features", "flooring"], v)
                }
              />
            </TopicSection>
            ...
            <TopicSection title="5. Financials" defaultOpen={true}>
              ...
              <EditableField
                ...
                onSave={(v) =>
                  handleFieldChange(
                    "acceptableFinancing",
                    ["financial", "acceptable_financing"],
                    v,
                  )
                }
              />
              ...
              <EditableField
                ...
                forceEdit={isEditMode}
                onSave={(v) =>
                  handleFieldChange("taxYear", ["financial", "tax_year"], v)
                }
              />
              <EditableField
              ...
              <EditableField
                ...
                forceEdit={isEditMode}
                onSave={(v) =>
                  handleFieldChange("taxRate", ["financial", "tax_rate"], v)
                }
              />
              <EditableField
                label="Occupant Type"
              ...
              <EditableField
                ...
                multiline={true}
                onSave={(v) =>
                  handleFieldChange("ownerName", ["showing", "owner_name"], v)
                }
              />
              <EditableField
                label="Lockbox Type"
