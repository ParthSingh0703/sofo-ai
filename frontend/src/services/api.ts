...
export const listingsApi = {
  ...
  openListingSite: async (listingId: string, mlsSystem: string, mlsUrl?: string): Promise<{ status: string; message: string }> => {
    const params = new URLSearchParams({ mls_system: mlsSystem });
    if (mlsUrl) {
      params.append('mls_url', mlsUrl);
    }
    return apiClient.post(`/automation/listings/${listingId}/open-site?${params.toString()}`);
  },

  getSessionStatus: async (listingId: string): Promise<{ is_active: boolean }> => {
    return apiClient.get(`/automation/listings/${listingId}/session-status`);
  },

  startAutomation: async (listingId: string, mlsSystem: string, mlsUrl?: string): Promise<AutomationResult> => {
    const params = new URLSearchParams({ mls_system: mlsSystem });
    if (mlsUrl) {
      params.append('mls_url', mlsUrl);
    }
    // Automation can take several minutes (manual login + form filling), so use longer timeout
    return apiClient.post(`/automation/listings/${listingId}/start?${params.toString()}`, undefined, {
      timeout: 600000 // 10 minutes timeout for automation
    });
  },
};
