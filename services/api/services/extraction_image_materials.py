...
def extract_materials_from_images(listing_id: UUID) -> Dict[str, ExtractedField]:
    ...
    from exterior property photos using AI vision analysis.
    
    Only processes images marked as exterior (front_exterior, back_exterior, side_exterior).
    ...
    """
    # Get only exterior images for this listing
    # Note: Images must be analyzed first (via enrichment) to identify exterior images
    images = _get_listing_images(listing_id)
    
    if not images:
        print("No exterior images found for material extraction. Images may need to be analyzed first.")
        return {}
    
    print(f"Extracting materials from {len(images)} exterior image(s)...")
...
def _get_listing_images(listing_id: UUID) -> List[Dict[str, Any]]:
    """
    Get only exterior images for a listing from database.
    Only processes images marked as exterior (front_exterior, back_exterior, side_exterior).
    """
    with get_db() as (conn, cur):
        cur.execute(
            ...
            AND (
                ia.detected_features->>'room_label' IN ('front_exterior', 'back_exterior', 'side_exterior')
                OR ia.detected_features->>'photo_type' = 'exterior'
            )
