        ...
        for mls_field_name, mapping in fields.items():
            ...
            if value is not None and mapping.transform_fn:
                ...
                try:
                    ...
                    if value != original_value and value is not None:
                        mapping_notes.append({
                            "mls_field": mls_field_name,
                    ...
                    # If transform fails, set value to None and log warning
                    print(f"Warning: Transform failed for {mls_field_name} (value: {original_value}): {str(e)}")
                    value = None
                    mapping_notes.append({
                        "mls_field": mls_field_name,
                        "canonical_source": mapping.canonical_path or "default",
...
def _apply_transform(value: Any, transform_name: str, canonical_dict: Dict[str, Any]) -> Any:
    ...
    if transform_name == "format_date":
        ...
        if isinstance(value, str):
            ...
            # If no numbers found and value is not numeric, return None
            # This handles cases where the field contains text like "Dining Room" instead of a number
            return None
        try:
