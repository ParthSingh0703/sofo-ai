...
from services.api.services.mls_automation.browser_session import (
    get_session,
    set_session,
    BrowserSession,
    is_session_active,
...
# Store playwright instances to keep browsers alive
_playwright_instances: Dict[UUID, Any] = {}
...
def open_listing_site(listing_id: UUID, mls_system_code: str, mls_url: Optional[str] = None) -> dict:
    ...
    Args:
        listing_id: The listing ID
        mls_system_code: MLS system code (e.g., "unlock_mls")
        mls_url: Optional MLS URL (for new MLS systems)
    ...
    if not PLAYWRIGHT_AVAILABLE:
        return {
            "status": "error",
            "message": "Playwright not installed. Please install with: pip install playwright && playwright install chromium"
        }
    
    # Check if session already exists
    if is_session_active(listing_id):
        return {
            "status": "already_open",
            "message": "Browser session is already open for this listing"
        }
    
    # Get MLS URL
    target_url = mls_url or _get_mls_url(mls_system_code)
    if not target_url:
        return {
            "status": "error",
            "message": "MLS URL not provided and not found in configuration"
        }
    ...
    try:
        ...
        set_session(listing_id, session)
        
        return {
            "status": "opened",
            "message": f"Browser opened at {target_url}. Please navigate and login, then click 'Start Automation' when ready."
        }
...
def start_automation(config: AutomationConfig) -> AutomationResult:
    ...
    # Check if browser session already exists (from open_listing_site)
    session = get_session(config.listing_id)
    ...
    try:
        ...
        try:
            ...
            result.login_skipped = True  # Always true since we skip login now
...
def _get_mls_url(mls_system_code: str) -> Optional[str]:
    ...
    known_urls = {
        "unlock_mls": "http://localhost:8080/"  # Placeholder
    }
